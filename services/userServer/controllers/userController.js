...
class Controller {
  static register = async (req, res, next) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ access_token: access_token, username: userData.username });
    } catch (error) {
      return next(error);
    }
  };
    ...
    try {
      ...
      if (!userLogin) {
        ...
        if (compareHash(password, userLogin.password)) {
          ...
          const access_token = generateToken(user);
          const update = await User.update(changeStatus, {
            where: { id: userLogin.id },
          });
          console.log(update);
          return res
            .status(200)
            .json({ access_token: access_token, username: userLogin.username });
        } else {
  ...
  static async logout(req, res, next) {
    try {
      ...
      const data = await User.update(changeStatus, {
